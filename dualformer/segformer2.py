...
class Scale_reduce(nn.Module):
    def __init__(self, dim, reduction_ratio):
        ...
        if len(self.reduction_ratio) == 4:
            ...
            self.sr00 = nn.Conv2d(dim * 2, dim * 2, reduction_ratio[2], reduction_ratio[2])
            self.sr11 = nn.Conv2d(dim * 5, dim * 1, reduction_ratio[1], reduction_ratio[1])
            self.sr22 = nn.Conv2d(dim * 8, dim * 1, reduction_ratio[0], reduction_ratio[0])

        self.norm = nn.LayerNorm(dim * 4)

    def forward(self, x0, x1, x2) -> torch.Tensor:
        # print(x0.shape)
        # print(x1.shape)
        # print(x2.shape)
        ...
        if len(self.reduction_ratio) == 3:
            # print('-----------')
            # sr_0 = torch.cat([self.sr00(x0).flatten(-2), self.sr10(x1).flatten(-2), self.sr20(x2).flatten(-2)], -1).transpose(-2, -1)
            # sr_1 = torch.cat([self.sr01(x0).flatten(-2), self.sr11(x1).flatten(-2), self.sr21(x2).flatten(-2)], -1).transpose(-2, -1)
            # sr_2 = torch.cat([self.sr02(x0).flatten(-2), self.sr12(x1).flatten(-2), x2.flatten(-2)], -1).transpose(-2, -1)

            # print(self.sr00(x0).shape)
            # print(self.sr11(x1).shape)
            # print(x2.shape)
            sr = torch.cat([self.sr00(x0).flatten(-2), self.sr11(x1).flatten(-2), self.sr22(x2).flatten(-2)], -2).transpose(-2, -1)
            # print(sr.shape)

        return self.norm(sr)


# (dims, head, reduction_ratios)
class M_EfficientSelfAtten(nn.Module):
    def __init__(self, dim, head, reduction_ratio):
        super().__init__()
        self.head = head
        self.reduction_ratio = reduction_ratio  # list[1  2  4  8]
        self.scale0 = (dim * 2 // head) ** -0.5
        self.scale1 = (dim * 5 // head) ** -0.5
        self.scale2 = (dim * 8 // head) ** -0.5

        self.q0 = nn.Linear(dim * 2, dim * 2, bias=True)
        self.kv0 = nn.Linear(dim * 4, dim * 4, bias=True)
        self.proj0 = nn.Linear(dim * 2, dim * 2)
        self.q1 = nn.Linear(dim * 5, dim * 5, bias=True)
        self.kv1 = nn.Linear(dim * 4, dim * 10, bias=True)
        self.proj1 = nn.Linear(dim * 5, dim * 5)
        self.q2 = nn.Linear(dim * 8, dim * 8, bias=True)
        self.kv2 = nn.Linear(dim * 4, dim * 16, bias=True)
        self.proj2 = nn.Linear(dim * 8, dim * 8)
    ...
    def forward(self, x0, x1, x2) -> torch.Tensor:
        ...
        x0, x1, x2 = x0.flatten(-2).transpose(-2, -1), x1.flatten(-2).transpose(-2, -1), x2.flatten(-2).transpose(-2, -1)
        # print('---x---')
        # print(x0.shape)
        # print(x1.shape)
        # print(x2.shape)
        ...
        q0 = self.q0(x0).reshape(B, N0, self.head, C0 // self.head).permute(0, 2, 1, 3)
        q1 = self.q1(x1).reshape(B, N1, self.head, C1 // self.head).permute(0, 2, 1, 3)
        q2 = self.q2(x2).reshape(B, N2, self.head, C2 // self.head).permute(0, 2, 1, 3)

        kv0 = self.kv0(sr).reshape(B, -1, 2, self.head, C0 // self.head).permute(2, 0, 3, 1, 4)
        kv1 = self.kv1(sr).reshape(B, -1, 2, self.head, C1 // self.head).permute(2, 0, 3, 1, 4)
        kv2 = self.kv2(sr).reshape(B, -1, 2, self.head, C2 // self.head).permute(2, 0, 3, 1, 4)
        k0, v0, k1, v1, k2, v2 = kv0[0], kv0[1], kv1[0], kv1[1], kv2[0], kv2[1]
        ...
        attn0 = (q0 @ k0.transpose(-2, -1)) * self.scale0
        attn1 = (q1 @ k1.transpose(-2, -1)) * self.scale1
        attn2 = (q2 @ k2.transpose(-2, -1)) * self.scale2
        # print(attn0.shape)
        # print(attn1.shape)
        # print(attn2.shape)
        # attn_score = attn.softmax(dim=-1)
        x_atten = (attn0.softmax(dim=-1) @ v0).transpose(1, 2).reshape(B, N0, C0)
        # print(x_atten.shape)
        out0 = self.proj0(x_atten)
        x_atten = (attn1.softmax(dim=-1) @ v1).transpose(1, 2).reshape(B, N1, C1)
        # print(x_atten.shape)
        out1 = self.proj1(x_atten)
        x_atten = (attn2.softmax(dim=-1) @ v2).transpose(1, 2).reshape(B, N2, C2)
        # print(x_atten.shape)
        out2 = self.proj2(x_atten)
